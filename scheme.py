...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        new_lamb = do_lambda_form(Pair( target.second, expressions.second),env)
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        '''new_frame = Frame(env.parent)

        if len(self.formals) != len(args):
            raise SchemeError
        for i in range(len(args)):
            new_frame.define(formals.first, args.first)
            formals = formals.second
            vals = vals.second
        return new_frame'''
